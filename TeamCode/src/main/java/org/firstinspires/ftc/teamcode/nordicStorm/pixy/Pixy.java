...
public class Pixy extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
     ...
     * you would have updateBlockList(). This object is exposed through
     * "getBlock()" which updates the block before handing it to the user.
     * this means that "updateBlock()" does not need to be called
     * by the user, and "getBlock()" will never throw a null pointer error.
     */
    private PixyBlock pixyBlock;
    private final EmptyBlock emptyBlock = new EmptyBlock();
    ...
    public int getFPS() {
        ...
        /*
        the FPS packet is returned as a 32-bit word, meaning we combine 4 bytes to get a readable value.
         */
